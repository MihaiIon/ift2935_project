...
public class RequestsController {
    ...
    void request1(){
        ...
        ArrayList<String> res =  clientController.getDataManager().request(String.format("with r1 as (select product_id as id from davidbam.offer where client_id = %d), \n" +
                ...
                "select id as seller_id, name as seller_name, product_id, product_name, seller_price, expert_id, expert_name, reputation from davidbam.seller natural join r4;",clientId));

        fillTable(res);
    }
    @FXML
    void request2(){
        ArrayList<String> res =  clientController.getDataManager().request(String.format("with r1 as (select product_id as id, amount, id as offer_id, offer_date from davidbam.offer where client_id = %d), \n" +
                ...
                "select product_id, product_name, id as seller_id, name as seller_name, seller_price, offer_id, amount, offer_date, expert_id, reputation  from davidbam.seller natural join r3;",clientId));
    ...
    @FXML
    void request3(){
        ArrayList<String> res =  clientController.getDataManager().request(String.format("with r1 as (select product_id from davidbam.offer where client_id =%d), \n" +
                "     r2 as (select product_id as id, client_id, id as offer_id, amount from davidbam.offer natural join r1 where client_id != %d)\n" +
                "select id as product_id, name as product_name, client_id, offer_id, amount from davidbam.product natural join r2 where state = 'AVAILABLE';",clientId, clientId));

        fillTable(res);
    }@FXML
    void request4(){
        ArrayList<String> res =  clientController.getDataManager().request(String.format("select id as product_id, name as product_name, seller_price, description \n" +
                "from davidbam.product \n" +
                "where id not in (select product_id from davidbam.offer where client_id = %d) and seller_price < 70 and state = 'AVAILABLE';",clientId));

        fillTable(res);
    }
    @FXML
    void request5(){
        ArrayList<String> res =  clientController.getDataManager().request(String.format("with r1 as (select client_id, offer_id as id from davidbam.transaction where client_id = %d),\n" +
                ...
                "select nb_offer, nb_accepted, total_paid from r3 natural join r2;",clientId));
